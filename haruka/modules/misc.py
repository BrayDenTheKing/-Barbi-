...
def roll(bot: Bot, update: Update):
	...
	try:
	    ...
	    if message.reply_to_message:
	        ...
	        bot.sendDice(chat_id=update.effective_chat.id)

	        return

	if match == "ð¯":

	    bot.sendDice(chat_id=update.effective_chat.id, emoji="ð¯")
...
def song(bot: Bot, update: Update):
	 ...
	 progress_message = update.effective_message.reply_text(f"Uploading {search_query} ð")
	 ...
	 opts = { 'format': 'bestaudio', 'addmetadata': True, 'key': 'FFmpegMetadata', 'writethumbnail': True, 'prefer_ffmpeg': True, 'geo_bypass': True, 'nocheckcertificate': True, 'postprocessors': [{ 'key': 'FFmpegExtractAudio', 'preferredcodec': 'mp3', 'preferredquality': '320', }], 'outtmpl': '%(id)s.mp3', 'quiet': True, 'logtostderr': False}
	 ...
	 bot.send_audio(chat_id=update.message.chat_id, audio=open(xxx, 'rb'), caption=t)

	 bot.delete_message(chat, progress_message.message_id)
